...
1.  **Deterministic Frame Sampling:** Input streams are sampled at exactly **10 FPS** using frame skipping to ensure temporal consistency with training data.
...
5.  **Bidirectional GRU Inference:** Classifies 32-frame motion buffers (~3 seconds) using a model that reads the sequence forward and backward for better context.
...
### Execution
1.  **Preprocessing:** `python src/preprocess.py` (Extracts frames at 10 FPS)
...
-   **Sampling Rate:** Strictly 10 FPS (deterministic).
